...
async function fixDatabase() {
  ...
  console.log('🔧 Corrigiendo base de datos con cifrado compatible...')
  console.log('')
  ...
  try {
    ...
    // 5. Crear datos de prueba CIFRADOS con el método correcto
    ...
    const mariaTasks = [
      ...
      { 
        user_id: maria.id, 
        title: encryptSensitiveData("Ejercicio matutino"), 
        description: encryptSensitiveData("Correr 30 minutos"), 
        category: "salud", 
        priority: "media", 
        status: "pendiente", 
        duration: 30, 
    ...
    // Tareas para Juan (CIFRADAS)
    const juanTasks = [
      { 
        user_id: juan.id, 
        title: encryptSensitiveData("Estudiar React"), 
        description: encryptSensitiveData("Repasar hooks y componentes"), 
        category: "estudio", 
        priority: "alta", 
        status: "en_progreso", 
        duration: 90, 
      ...
      { 
        user_id: juan.id, 
        title: encryptSensitiveData("Proyecto personal"), 
        description: encryptSensitiveData("Desarrollar portafolio web"), 
        category: "personal", 
        priority: "media", 
        status: "pendiente", 
        duration: 120, 
    ...
    const mariaMoods = [
      { 
        ...
        hour: 8, 
        date: yesterday, 
        notes: encryptSensitiveData("Muy buena mañana, me siento productiva") 
      },
      { 
        user_id: maria.id, 
        energy: 5, 
        ...
        hour: 10, 
        date: yesterday, 
        notes: encryptSensitiveData("Máxima concentración en el trabajo") 
      },
      { 
        user_id: maria.id, 
        energy: 4, 
        ...
        hour: 9, 
        date: today, 
        notes: encryptSensitiveData("Listo para el día, con muchas ganas") 
      },
    ]
    ...
    const juanMoods = [
      { 
        ...
        hour: 14, 
        date: today, 
        notes: encryptSensitiveData("Día normal, estudiando programación") 
      },
    ]

    for (const mood of juanMoods) {
      await client.execute({
        sql: "INSERT INTO moods (user_id, energy, focus, stress, type, hour, date, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        args: [mood.user_id, mood.energy, mood.focus, mood.stress, mood.type, mood.hour, mood.date, mood.notes],
      })
    ...
    // 7. Probar que el cifrado funciona
    ...
    console.log('3. Los datos están cifrados y son compatibles con la app')
